...
const CodePageClient = () => {
  ...
  const handleClickPrevious = () => {
    ...
    const href = `/code/${practicalSubject?.key}/${
      practicalSubject?.practicals.find((_, i) => i == oldIndex - 1)?.key
    }`;
    router.push(href);
  };
      ...
      >
        ...
        >
          {practicalSubject?.practicals?.map((ele) => (
            ...
            >
              <Link
                href={`/code/${practicalSubject.key}/${ele.key}`}
        ...
        <Button
          ...
          className="max-md:hidden"
          disabled={
            practicalSubject?.practicals?.length == currentPracticalIndex + 1
          }
